...
def handle_text(message):
    if message.chat.type == 'private':
        ...
        def callback_inline(call):
            if call.data == '1':
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="Внимательно изучи проблемные ситуации на маршруте. Пиши боту @idivgopuanyaBot - *проблема*.\n"
    ...
    if message.chat.type == 'private':
        if message.text == "Контакты":
            ...
            bot.send_message(message.chat.id, "🔸*Баллы за доставленный заказ*\n"
                                              ...
                                              "🔸*-6 баллов за перенос/недозвон день в день*\n\n"
                                              "` - таблица с баллами и основными показателями доступна у `[Бота](https://t.me/idivgopuanyaBot)`. Пиши `*баллы*", parse_mode="Markdown")
...
добавлен счетчик отправленных чеков, чтобы видеть такие зависшие чеки.`""", parse_mode="Markdown")
            ...
            def callback_inline(call):
                if call.data == 's_klientom':
                    ...
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text="Пиши [Боту](https://t.me/idivgopuanyaBot)- *как включить планшет*", parse_mode="Markdown")
                elif call.data == 'rider':
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text="Пиши [Боту](https://t.me/idivgopuanyaBot)- *как подключить ридер*", parse_mode="Markdown")
