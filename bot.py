...
zakaz_button_exit = types.InlineKeyboardButton("🚫", callback_data='zakaz_exit')
...
"6️⃣*Статусы заказов в LEOS*\nВсе статусы в ЛЕОС соответствовали данным, полученным от клиента. Если записей с отменой или переносом нет - это 0", parse_mode="Markdown")
            ...
            def callback_inline(call):
                if call.data == 's_klientom':
                    ...
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text="Включи впн, обнови заказы. Позвони в Мокка, чтобы уточнить статус оплаты.\nКак включить впн и узнать номер Мокка ты можешь у [Бота](https://t.me/lamodadedbot). Пиши *впн* и *номер мокка*", parse_mode="Markdown", reply_markup=nazad_menu)
                elif call.data == 'zavis':
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text=constants.bezcheka, parse_mode="Markdown", reply_markup=nazad_menu)
                elif call.data == 'nevkl':
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text="Пиши [Боту](https://t.me/lamodadedbot)- *как включить планшет*", parse_mode="Markdown", reply_markup=nazad_menu)
                elif call.data == 'rider':
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text="Пиши [Боту](https://t.me/lamodadedbot)- *как подключить ридер*", parse_mode="Markdown", reply_markup=nazad_menu)
...
`Посмотри внимательно, скорее всего 1 позиции в заказе не хватает электронный акт недостача`""", parse_mode="Markdown")
            ...
            bot.send_message(message.chat.id, "[Баллы Март](https://docs.google.com/spreadsheets/d/13_S1ojhmj7GzrrlMnSEaysZRZGJzxZkE8Q2yk1q_0xY/edit#gid=479252611)", parse_mode="Markdown")
        ...
        if message.chat.title == "🚛Lamoda info👟" or message.chat.title == "Lamoda Dedovik":
            if "привет" in message.text.lower() and "бот" in message.text.lower():
                if "проблем" in message.text.lower():
                    ...
                    bot.send_message(message.chat.id, "[Баллы](https://docs.google.com/spreadsheets/d/13_S1ojhmj7GzrrlMnSEaysZRZGJzxZkE8Q2yk1q_0xY/edit#gid=479252611)", parse_mode="Markdown")
                elif "дежурн" in message.text.lower() and "механик" not in message.text.lower():
                    bot.reply_to(message, "*Дежурный СВ* +79160558030", parse_mode="Markdown")
                elif "мокка" in message.text.lower() or "мокко" in message.text.lower():
                    bot.reply_to(message, "*Мокка* +78007077236", parse_mode="Markdown")
